...
class Ship:
    def __init__(self, file, crane_position, jib_length, TP_position, TP_amount, hull_thickness, BHD_thickness, tank3_initial, slewing_angle, jib_angle, TP_mass=230000, water_density=1025, material_density=7850, mass_factor=2.1):
        ...
        #main particulars
        self.main_data = json.load(open(f'data/MainShipParticulars_Gr{self.file[0]}_V{self.file[1]}.{self.file[2]}.json'))

        #dimensions
        self.LOA = self.main_data['MAIN DIMENSIONS']['Loa_m']
        self.width = self.main_data['MAIN DIMENSIONS']['B_m']
        self.height = self.main_data['MAIN DIMENSIONS']['H_m']
        self.LPP = self.main_data['MAIN DIMENSIONS']['Lpp_m']
        self.COB = self.main_data['VOLUME RELATED DATA (MOULDED)']['COB_m']
        self.COV = self.main_data['VOLUME RELATED DATA (MOULDED)']['COV_Total_m']
        self.buoyant_volume = self.main_data['VOLUME RELATED DATA (MOULDED)']['Buoyant_Volume_m3']
        self.I = self.main_data['DATA OF UNDERWATER AREAS (MOULDED)']['Inertia_WPA_around_COF_m4']
