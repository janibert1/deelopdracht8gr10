...
class Tank:
    def __init__(self, file1, file2, water_density, buoyant_volume, COV):
        #data
        self.volume_data = pd.read_csv(file1, delimiter=',', skiprows=1)
        self.waterplane_data = pd.read_csv(file2, delimiter=',', skiprows=1)
        #volume data
...
def deck(crane_boolean, crane_position, TP_amount, TP_position, TP_mass, jib_length, jib_angle, slewing_angle): #positions(COG) = [x,y,z]
    #degrees to radians
    jib_angle = (jib_angle/180)*math.pi
    slewing_angle = (slewing_angle/180)*math.pi
    
    cargo_mass = TP_amount*TP_mass
    cargo_lcg = TP_position[0]
    cargo_tcg = TP_position[1]
    cargo_vcg = TP_position[2]
    ...
    if crane_boolean:
        ...
        mass_deck = np.array([cargo_mass])
        lcg_deck = np.array([cargo_lcg])
        tcg_deck = np.array([cargo_tcg])
        vcg_deck = np.array([cargo_vcg])
    
    return np.array([mass_deck, lcg_deck, tcg_deck, vcg_deck])

def plates(file, hull_thickness, BHD_thickness, material_density, mass_factor): #file = [group, version, subversion]
